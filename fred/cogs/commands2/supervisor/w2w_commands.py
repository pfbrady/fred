...
class W2WCommands(discord.app_commands.Group):
    def __init__(self, name, description, fred):
        super().__init__(name=name, description=description)
        self.fred: Fred = fred
        self.guards_default_times = ['now', 'earlier-today', 'later-today', 'today', 'today-closers', 'tomorrow',
                                     'tomorrow-openers', 'tomorrow-closers', 'week', 'week-openers', 'week-closers']
    ...
    def get_shifts_from_auto(int_branch: Branch, position_auto: str, time_auto: str) -> List[Shift]:
        ...
        positions: List[Position] = [int_w2w_client.specialist, int_w2w_client.supervisor]
        for pool_group in int_branch.pool_groups:
    ...
    @discord.app_commands.describe(
        time="The time group which you intend to send a message to. Options are listed above.")
    ...
    async def guards(self, interaction: discord.Interaction, time: str, position: str, message: str):
        ...
        if discord_users:
            await interaction.response.send_message(
                f"Notification: {' '.join([user.mention for user in discord_users])}: {message}")
        else:
            await interaction.response.send_message(
                f"No lifeguards working at the indicated time. Please adjust your parameters.", ephemeral=True)
    ...
    @discord.app_commands.command(
        description="Sends a notification to a group of swim instructors, filtered by time and position")
    async def instructors(self, interaction: discord.Interaction, time: str, position: str, message: str):
        ...
        if discord_users:
            ...
            await interaction.response.send_message(
                f"No swim instructors working at the indicated time. Please adjust your parameters.", ephemeral=True)


async def setup(fred: Fred):
    fred.tree.add_command(W2WCommands(name="w2w",
                                      description="Commands for fetching information from WhenToWork and sending "
                                                  "messages to those who are working.",
                                      fred=fred))
